...
def showOriSignal(sample, label, idx):
    ''' 绘制并展示一个样本信号的图像 '''
    mod_name = "label-"+str(t2n(label))
    signal_data = sample[1]

    plt.figure(figsize=(18, 12))
    plt.title(mod_name+"-"+str(idx))
    plt.xlabel('N')
    plt.ylabel("Value")

    plt.plot(signal_data[:, 0], label = 'sig')
    plt.legend(loc="upper right")

    # save_path = "figs_fea_HRRP/{}".format(mod_name)
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)
    # plt.savefig(save_path + '/' + str(idx+1))
    # plt.close()
...
def showCamSignal(signal, CAM, label, idx, layer):
    """
    Args:
    ...
    plt.title(mod_name+"_Conv"+str(layer))
    plt.xlabel('N')
    plt.ylabel("Value")

    plt.plot(signal_data[:, 0]*(sig_len//10), label = 'hrrp')
    # plt.plot(signal_data[:, 0]*(sig_len//10), color = 'red', label = 'Q')
    plt.legend(loc="upper right")
    ...
    save_path = "figs_CAM_HRRP/{}".format(mod_name)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    plt.savefig(save_path + '/' + str(idx) +"_Conv"+ str(layer) +"_CAM")
