...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
   <1> | JunJie Ren |   v1.1    | 2020/08/23 | 增加星座图绘制
--------------------------------------------------------------------------
'''
...
# from dataset.RML2018.classes import modName

class RMLDataset(Dataset):
    ''' 定义RMLDataset类，继承Dataset方法，并重写__getitem__()和__len__()方法 '''
    def __init__(self, data_root, data_label, transform=None, constellation=False):
        ''' 初始化函数，得到数据 '''
        self.data = data_root
    ...
    def __getitem__(self, index):
        ...
        if self.constel:
            ...
            data = np.swapaxes(data, 0, 1)            # temp
            plt.scatter(data[0, :, 0], data[0, :, 1], c='black', marker='.', label=modName[labels])
            plt.axis('off')
            fig = plt.gcf()
            img = fig2im(fig)
            plt.legend()        # temp
...
def fig2im(fig):
    ...
    im = cv2.resize(im, (224, 224))     # [224, 224, 3]
...
if __name__ == "__main__":
    ...
    torch_data = RMLDataset(x_train, y_train, transform=transform, constellation=False)
    # 通过DataLoader读取数据
    ...
    #     print(data[0].shape)        # [bz, 3, 224, 224]
    #     exit()
