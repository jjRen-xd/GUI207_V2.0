...
void ImageWidget::saveFig()
{
    QString filename = QFileDialog::getSaveFileName(NULL, "保存当前图片","./","Images (*.png *.bmp *.jpg)"); //选择路径
    if(filename == ""){
        QMessageBox::warning(NULL, "提示", "图像保存失败!");
    }
    else{
        m_pix.save(filename, Q_NULLPTR, 100);
        QMessageBox::information(NULL, "提示", "图像保存成功！");
    }
}