#include "matdataprocess_afs.h"

MatDataProcess_afs::MatDataProcess_afs(std::vector<int> dataOrder,int modelIdx):
    dataOrder(dataOrder),modelIdx(modelIdx){
    
}
//TODO 重名可能有问题
void MatDataProcess_afs::oneNormalization(std::vector<float> &list){
    //特征归一化
...
void MatDataProcess_afs::getAllDataFromMat(std::string matPath,bool dataProcess,std::vector<std::vector<float>> &data,std::vector<int> &labels,int label,int inputLen){
    ...
    if(!pMatFile){
        qDebug()<<"(MatDataProcess_afs:getAllDataFromMat)文件指针空！！！！！！";
        return;
    }
    ...
    if(!pMxArray){
        qDebug()<<"(MatDataProcess_afs:getAllDataFromMat)pMxArray变量没找到！！！！！！";
        return;
    }
    ...
    //for(int i=N/2;i<N;i++){
        ...
        //     std::cout<<std::endl<<"(MatDataProcess_afs::getAllDataFromMat)Cone.mat the "<<i<<"col data:"<<std::endl;
        ...
        if(modelIdx>M){
            modelIdx=M;
            qDebug()<<"(MatDataProcess_afs::getAllDataFromMat) 欲选择的特征数量大于实际最大,自适应选取所有";
        }
...
void MatDataProcess_afs::getDataFromMat(std::string targetMatFile,int emIdx,bool dataProcess,float *data,int inputLen){
    MATFile* pMatFile = NULL;
    mxArray* pMxArray = NULL;
    // 读取.mat文件（例：mat文件名为"initUrban.mat"，其中包含"initA"）
    double* matdata;
    ...
    if(!pMatFile){
        qDebug()<<"(MatDataProcess_afs:getDataFromMat)文件指针空！！！！！！";
        return;
    }
    ...
    qDebug()<<"(MatDataProcess_afs:getDataFromMat)matVariable=="<<QString::fromStdString(matVariable);
    pMxArray = matGetVariable(pMatFile,matVariable.c_str());
    if(!pMxArray){
        qDebug()<<"(MatDataProcess_afs:getDataFromMat)pMxArray变量没找到！！！！！！";
        return;
    }
    matdata = (double*)mxGetData(pMxArray);
    int M = mxGetM(pMxArray);  //行数
