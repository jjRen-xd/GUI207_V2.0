...
from dataset.HRRP_mat import HRRPDataset
...
def train():
   ''' 信号调制分类训练主程序 '''
   # model
   model = CNN_HRRP512(num_classes=cfgs.num_classes)
   print(model)
   model.cuda()
   ...
   # Train data
   train_dataset = HRRPDataset(cfgs.trainMatPath, transform=transform) 
   dataloader_train = DataLoader(train_dataset, \
                                 ...
                                 drop_last=True)
   # Valid data
   valid_dataset = HRRPDataset(cfgs.validMatPath, transform=transform)
